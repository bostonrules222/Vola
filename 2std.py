...
for stock in stock_list:
	try:
		pull = quandl.get('VOL/' + stock, column_index='27', start_date=yesterday, end_date=yesterday)
		sp = quandl.get('EOD/' + stock, column_index='4', start_date=yesterday, end_date=today)
		price = sp[['Close']].to_numpy()
		Iv30 = pull[['IvMean30']].to_numpy()
		std = (Iv30[0][0]/math.sqrt(252)) * price[0][0]
		move_calc = abs((price[1][0] - price[0][0])/std)
		if move_calc > 2:
			top_dct[stock] = round(float(move_calc),2)
	except:
		pass
